...
1. Explain the difference between `RDBMS` and `SQL`.
  The Relational Database Management System manages storage, querying, updating, ect. 
  
  SQL is the language used to interact with the RDBMS.

1. Why do tables need a `primary key`?
  Primary keys are a unique integer that we use to index columns and are used as a referance. 

1. What is the name given to a table column that references the primary key on another table.
  Foreign key.

1. What do we need in order to have a _many to many_ relationship between two tables.
  One table has to have rows with relationships with multiple rows in the another table and vice versa.
